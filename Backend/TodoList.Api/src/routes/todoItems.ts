...
const create = async (newTodoItem: TodoItem): Promise<TodoItem> => {
    const uid = new ShortUniqueId({length: 10});
    const id: string = newTodoItem.id ? newTodoItem.id : uid(); // post item with id (postman)
    todoItems[id] = {
        id,
        ...newTodoItem,
    };
...
const update = async (id: string, todoItemUpdate: BaseTodoItem): Promise<TodoItem | null> => {
...
todoItemRouter.post("/", async (req: Request, res: Response) => {
    try {
        ...
        const validation = await todoItemValidation(todoItem, res);
        if (validation) {
            return validation;
        }
...
const todoItemValidation = async (todoItem: BaseTodoItem, res: Response) => {
    ...
    if (FORBIDDEN_WORDS.indexOf(todoItem?.description.toLowerCase()) > -1) {
        return res.status(400).send(`The word "${description}" is forbidden`);
    }

    return false;
}

const todoItemDescriptionExists = async (description: string): Promise<boolean> => {
    return Object.values(todoItems).some((todoItem) => todoItem.description === description);
};